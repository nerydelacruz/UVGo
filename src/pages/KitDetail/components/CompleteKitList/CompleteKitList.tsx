...
import { useSaveKit } from "@/hooks/useSaveKit"
...
const CompleteKitList = ({ kit }: { kit: KitWithArticulos }) => {
  ...
  const { mutate, isPending, isError } = useSaveKit(kit)
  ...
  const handleSave = () => {
    ...
    mutate(
      ...
      {
        onSuccess: (kitGuardado) => {
          setNuevos([])
          // el kit base no se edita: guardar crea una copia personalizada nueva, hay que ir a verla
          if (!yaEsPersonalizado) {
            navigate(`/kits/${kitGuardado.kitId}`)
          }
        },
      },
    )
